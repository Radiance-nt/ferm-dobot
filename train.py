        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                if (args.agent == 'curl_sac' and args.encoder_type == 'pixel') or \
                        (args.agent == 'rad_sac' and (
                                args.encoder_type == 'pixel' or 'crop' in args.data_augs or 'translate' in args.data_augs)):
...
def main():
    ...
    exp_name += '-' + ts + '-' + env_name + '-im' + str(args.image_size) + '-b' + str(args.batch_size) + '-nu' + str(
        args.num_updates)
    if args.observation_type == 'hybrid':
        exp_name += '-hybrid'
    ...
    video = VideoRecorder(video_dir, camera_id=args.cameras[0])
    ...
    if args.demo_model_dir is not None:  # collect demonstrations using a state-trained expert
        ...
        # Collecting expert trajectories into Replay_Buffer
        while t < args.demo_samples:
            ...
            action = agent.sample_action(state_obs)
    ...
    if args.warmup_offline_sac:
        print("Warming up offline_sac for " + str(args.warmup_offline_sac) + ' steps.')
        for i in range(args.warmup_offline_sac):
            agent.update_sac_only(replay_buffer, L, step=0)
        print('Warmed up offline_sac.')

    if args.bc_only:
        step = 0
        print("Warming up bc for " + str(100) + ' steps ONLY.')
        for i in range(100):
            agent.train_bc(replay_buffer)
            step += 1
        eval_and_save()
        print('Warmed up bc ONLY.')
        return
    ...
    if args.synch_update:
        callback_fn = lambda: lambda: [
            agent.update(replay_buffer, L, step, log_networks=nu == 0 and step % args.log_networks_freq == 0) for nu in
            range(
                args.num_updates)] if step >= args.init_steps and not is_eval else 0  # pointers should all work properly, and execute in the proper frame
