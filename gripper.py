...
class RobotiqGripper(mm.Instrument):
    ...
    def __init__(self, portname, slaveaddress=9):
        ...
        self.client = ModbusSerialClient(method='rtu', port=portname, stopbits=1, bytesize=8, baudrate=115200,
                                         timeout=0.2)
        if not self.client.connect():
            print("Unable to connect to %s" % portname)
    ...
    def goToRel(self, x, speed=10, force=255):
        __max__ = 200
        __min__ = 20
        x = min(__max__, self.getPositionCurrent()[0] + x)
        x = max(__min__, x)
        self.goTo(x, speed, force)
